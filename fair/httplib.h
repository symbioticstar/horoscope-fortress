...
struct Request {
  ...
  std::string get_param_value_required(const char *key, size_t id = 0) const;
...
inline std::string Request::get_param_value_required(const char *key, size_t id) const {
    auto rng = params.equal_range(key);
    auto it = rng.first;
    std::advance(it, static_cast<ssize_t>(id));
    if (it != rng.second) { return it->second; }
    throw std::invalid_argument(std::string(key) + " not found");
}
